            ...
            <TouchableOpacity
                onPress={() =>
                    router.push({
                        ...
                        params: { user: JSON.stringify(author) },
                    })
                }
                style={{ marginLeft: 6 }}
            >
                {thumbnailSize === 'big' ? (<ProfileThumbnail user={author} />)
                    : author ? (<ProfileThumbnailSmall user={author} />) : (<Text>No seller information available</Text>)
                }
            </TouchableOpacity>
        </View>
