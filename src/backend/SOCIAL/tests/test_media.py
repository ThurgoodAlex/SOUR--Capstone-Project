...
def test_uploading_invalid_media(
    ...
    assert data["detail"] == 'file with MIME type text not accepted'
...
def test_uploading_invalid_post_id(
    ...
    assert response.status_code == 400 
    data = response.json()
    assert data["detail"] == f"Cannot find post associated with id: {invalid_post_id}"
...
def test_uploading_an_empty_file(
    ...
    assert response.status_code == 400 
    data = response.json()
    assert data["detail"] == "Empty file not accepted"
